...
/** Mutex: prevents concurrent pullFromCloud calls */
...
async function supabaseGet(table: string, params: string = "", signal?: AbortSignal): Promise<any[]> {
 ...
 * Uses mutex to prevent concurrent calls.
 */
export async function pullFromCloud(forceFull = false, signal?: AbortSignal): Promise<SyncResult> {
  ...
  try {
    ...
    const meta = await supabaseGet("vs_sync_meta", "select=last_webhook_at,total_pushes&id=eq.main", signal);
    ...
    // Fetch metrics + sleep in parallel
    const [rawMetrics, rawSleep] = await Promise.all([
      supabaseGet(
        "vs_health_metrics",
      ...
      supabaseGet(
        "vs_sleep_nights",
        `select=date,duration,deep_min,core_min,rem_min,awake_min,efficiency,bedtime,waketime,midpoint,segments${dateFilter}&order=date.asc&limit=1000`,
        signal
      ),
    ]);
    ...
    // Merge with existing data in IndexedDB (saveHealthData uses put = upsert)
...
export async function getWebhookInfo(): Promise<{ url: string; token: string } | null> {
  try {
    const meta = await supabaseGet("vs_sync_meta", "select=api_token&id=eq.main");
